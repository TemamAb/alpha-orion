...
const EnterpriseDashboard: React.FC = () => {
  ...
  const handleKillSwitch = () => {
    if (window.confirm("⚠️ EMERGENCY: Are you sure you want to STOP all trading?")) {
      setSystemStatus('EMERGENCY');
      // API call to stop trading would go here
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className={`p-4 rounded border ${['IDLE', 'VALIDATING', 'VALIDATION_FAILED'].includes(missionStep) ? 'border-blue-500 bg-blue-900/20' : 'border-gray-700 bg-gray-800/50'}`}>
              ...
              {missionStep === 'VALIDATION_FAILED' && <div className="mt-2 text-red-400 text-xs font-bold">⚠️ Validation failed. Check logs for details.</div>}
            </div>
            ...
            <div className={`p-4 rounded border ${['VALIDATED', 'DEPLOYING', 'DEPLOYMENT_FAILED'].includes(missionStep) ? 'border-purple-500 bg-purple-900/20' : 'border-gray-700 bg-gray-800/50'}`}>
              ...
              {missionStep === 'DEPLOYMENT_FAILED' && <div className="mt-2 text-red-400 text-xs font-bold">⚠️ Deployment failed. Check logs for details.</div>}
              {missionStep === 'LIVE' && (
